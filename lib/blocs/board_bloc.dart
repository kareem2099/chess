...
class BoardBloc {
  ...
  BoardBloc({required chess_pkg.Chess chessGame})
  ...
  void movePiece(int curIndex, int newIndex) {
    ...
    if (_nextMoveNf.value == PieceColor.black) {
      ...
      _stockfish.stdin = 'position fen ';

      // Ask Stockfish to generate a move (e.g., 3 seconds per move)
      _stockfish.stdin = 'go movetime 3000';

      // Listen for the best move from Stockfish
      _stockfish.stdout.listen((move) {
